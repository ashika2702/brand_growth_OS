  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="space-y-4">
        <label className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-accent-blue ml-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* Dos */}
        <div className="p-6 bg-white/5 border border-white/5 rounded-[2rem] space-y-4 backdrop-blur-xl relative overflow-hidden group">
          ...
          <div className="flex flex-wrap gap-2">
            {value.vocab_do.map((word, idx) => (
              <span key={idx} className="px-3 py-1.5 bg-accent-green/10 text-accent-green text-[10px] font-black uppercase tracking-tight rounded-lg border border-accent-green/20 flex items-center gap-2 group/tag">
                {word}
                <button onClick={() => removeItem('vocab_do', idx)} className="hover:text-white transition-colors">
                  <XCircle size={10} />
                </button>
              </span>
            ))}
            <input 
              ...
              className="bg-transparent border-none outline-none text-[10px] font-medium text-white placeholder:text-slate-700 w-24"
            />
          </div>
        </div>
        ...
        <div className="p-6 bg-white/5 border border-white/5 rounded-[2rem] space-y-4 backdrop-blur-xl relative overflow-hidden group">
          ...
          <div className="flex flex-wrap gap-2">
            {value.vocab_dont.map((word, idx) => (
              <span key={idx} className="px-3 py-1.5 bg-accent-red/10 text-accent-red text-[10px] font-black uppercase tracking-tight rounded-lg border border-accent-red/20 flex items-center gap-2 group/tag">
                {word}
                <button onClick={() => removeItem('vocab_dont', idx)} className="hover:text-white transition-colors">
                  <XCircle size={10} />
                </button>
              </span>
            ))}
      ...
      {/* Adjectives & Style */}
      <div className="p-6 bg-white/5 border border-white/5 rounded-[2rem] space-y-4 backdrop-blur-xl">
        <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 flex items-center gap-2">
          <Sparkles size={12} className="text-accent-yellow" /> Brand Adjectives
        </label>
        <p className="text-[10px] text-slate-600 font-medium">Add 3-5 adjectives that describe your brand's aesthetic and personality.</p>
        <div className="flex flex-wrap gap-2">
          {value.adjectives.map((adj, idx) => (
            <span key={idx} className="px-4 py-2 bg-white/5 text-white/80 text-[10px] font-black uppercase tracking-[0.1em] rounded-xl border border-white/10 hover:border-accent-yellow/50 transition-all flex items-center gap-2">
          ...
          <input 
            ...
            className="flex-1 bg-white/5 border border-white/5 rounded-xl px-4 py-2 text-[10px] font-medium text-white outline-none focus:border-white/20 transition-all placeholder:text-slate-700"
          />
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <textarea
          ...
          className="w-full bg-white/5 border border-white/10 rounded-[2rem] p-6 text-xs h-32 outline-none focus:border-accent-blue/50 focus:bg-white/10 transition-all text-white placeholder:text-slate-700 leading-relaxed font-medium"
