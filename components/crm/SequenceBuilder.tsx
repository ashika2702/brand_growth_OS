        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex items-center justify-between">
                <div className="space-y-1">
                    <h2 className="text-xl font-black text-white uppercase italic tracking-tight">Neural Sequence Architect</h2>
                    <p className="text-slate-500 text-[10px] font-medium uppercase tracking-widest">Design multi-day autonomous outreach patterns.</p>
                </div>
            ...
            <div className="grid grid-cols-3 gap-8">
                ...
                <div className="col-span-1 space-y-6">
                    <div className="glass-card p-6 rounded-[2rem] border border-white/5 space-y-4">
                        <div className="space-y-2">
                            <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 ml-1">Sequence Name</label>
                            <input
                                value={name}
                        ...
                        <div className="space-y-2">
                            <label className="text-[9px] font-black uppercase tracking-widest text-slate-500 ml-1">Universal Goal</label>
                            <textarea
                                value={description || ''}
                                onChange={(e) => setDescription(e.target.value)}
                                placeholder="What is the final conversion point?"
                                className="w-full bg-white/5 border border-white/5 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-700 outline-none h-24 focus:border-accent-blue/30 transition-all"
                            />
                        </div>
                    </div>
                ...
                <div className="col-span-2 space-y-4">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">Execution Timeline</span>
                        <button
                            onClick={addStep}
                            className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-accent-orange hover:text-white transition-colors"
                        >
                    ...
                    <div className="space-y-4 relative">
                        ...
                        {steps.map((step, idx) => (
                            <div key={idx} className="relative flex gap-6 group animate-in slide-in-from-left-4 duration-500" style={{ animationDelay: `${idx * 100}ms` }}>
                                <div className="w-12 h-12 rounded-2xl bg-black border border-white/10 flex items-center justify-center text-[10px] font-black text-accent-blue shadow-2xl relative z-10 group-hover:border-accent-blue/50 transition-colors">
                                    {idx + 1}
                                </div>

                                <div className="flex-1 glass-card p-6 rounded-[2rem] border border-white/5 space-y-4 group-hover:border-white/10 transition-colors">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-4">
                                            <div className="flex items-center gap-2 bg-white/5 px-3 py-1.5 rounded-lg border border-white/5">
                                                ...
                                                />
                                                <span className="text-[9px] font-black uppercase text-slate-500">Days Delay</span>
                                            </div>
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-[8px] font-black uppercase tracking-widest text-slate-600 ml-1">AI Outreach Strategy</label>
                                        <textarea
                                            value={step.strategy}
                                            onChange={(e) => updateStep(idx, { strategy: e.target.value })}
                                            placeholder="What should the AI talk about in this step?"
                                            className="w-full bg-white/5 border border-white/5 rounded-xl px-4 py-3 text-xs text-white placeholder-slate-700 outline-none h-20 focus:border-accent-blue/30 transition-all font-medium italic"
                                        />
                                    </div>
                                </div>
                                {idx < steps.length - 1 && (
