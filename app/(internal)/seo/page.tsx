  ...
  return (
    <div className="h-full flex flex-col gap-6 overflow-y-auto no-scrollbar pb-8">
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="flex gap-3">
          <button className="px-6 py-3 bg-white/5 hover:bg-white/10 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest border border-white/10 transition-all flex items-center gap-2">
            <Zap size={14} className="text-teal-400" /> Run Full Audit
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'Organic Traffic', value: '8.9K', trend: '+12%', color: 'text-teal-400' },
          { label: 'Avg. Position', value: '4.2', trend: '-0.5', color: 'text-blue-400' },
          { label: 'Domain Rating', value: '68', trend: '+2', color: 'text-purple-400' },
          { label: 'Backlinks', value: '1.4K', trend: '+150', color: 'text-teal-400' },
        ].map((stat, i) => (
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-12 lg:col-span-8 glass-card p-8 rounded-[2.5rem] border border-white/5 min-h-[400px] flex flex-col">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white flex items-center gap-2">
              <BarChart3 size={14} className="text-teal-400" /> Visibility Index
            </h3>
            <div className="flex gap-2">
              <span className="w-3 h-3 rounded-full bg-teal-500/20 border border-teal-500/50" />
          ...
          <div className="flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={SEO_PERFORMANCE}>
                ...
                />
                <Area type="monotone" dataKey="organic" stroke="#14B8A6" strokeWidth={3} fillOpacity={1} fill="url(#colorOrganic)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="col-span-12 lg:col-span-4 glass-card p-8 rounded-[2.5rem] border border-white/5 flex flex-col">
          <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white mb-8 flex items-center gap-2">
            <ShieldCheck size={14} className="text-purple-400" /> Site Health Score
          </h3>
          ...
          <div className="flex flex-col items-center justify-center mb-10">
            <div className="relative w-32 h-32 flex items-center justify-center">
               <svg className="w-full h-full transform -rotate-90">
                 ...
                 <circle cx="64" cy="64" r="56" stroke="#8B5CF6" strokeWidth="12" fill="transparent" strokeDasharray="351.8" strokeDashoffset="42.2" strokeLinecap="round" />
               </svg>
          ...
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="flex justify-between items-center p-3 rounded-xl bg-white/5 border border-white/5 group hover:border-purple-500/30 transition-all">
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{item.label}</span>
                <span className={`text-[10px] font-black ${item.ok ? 'text-teal-400' : 'text-purple-400'}`}>{item.status}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="col-span-12 glass-card p-8 rounded-[2.5rem] border border-white/5 overflow-hidden">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-white flex items-center gap-2">
              <TrendingUp size={14} className="text-blue-400" /> Keyword Dominance
            </h3>
                  ...
                  <tr key={i} className="hover:bg-white/5 transition-colors group cursor-pointer">
                    <td className="py-4 px-4">
                      <div className="flex items-center gap-2">
                        ...
                        <ArrowUpRight size={12} className="text-slate-700 group-hover:text-teal-400 transition-colors" />
                      </div>
                    </td>
                    <td className="py-4 px-4 text-xs font-black text-slate-400">{row.vol}</td>
                    <td className="py-4 px-4">
                      <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded border ${
                        row.diff === 'Low' ? 'text-teal-400 border-teal-500/20' : 
                        row.diff === 'Med' ? 'text-blue-400 border-blue-500/20' : 
